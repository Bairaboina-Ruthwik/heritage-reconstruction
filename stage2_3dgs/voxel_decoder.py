 # Run this cell SECOND to create voxel_decoder.py
...
class VoxelDecoder(nn.Module):
    """
    Voxel-Decoder Hybrid Gaussian Representation.
    Equations 11 and 12 from the paper.
    """
...
def initialize_voxels_from_pointcloud(points, voxel_size=0.05, embed_dim=32):
    """Convert Dense-SfM point cloud into voxel grid."""
...
if __name__ == '__main__':
    dummy_points = np.random.randn(1000, 3) * 2.0
    voxel_data = initialize_voxels_from_pointcloud(dummy_points, voxel_size=0.1)
    print(f"Voxel centers shape: {voxel_data['centers'].shape}")
    print("✅ VoxelDecoder test passed!")